...
const Solarpanels =db.solarpanels;
...
//CREATE
const solarpanels=new Solarpanels({
...
.catch(err => {
 ...
 Solarpanels.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
 .then(data => {
 if (!data) {
 res.status(404).send({
...
exports.delete = (req, res) => {
 ...
 Solarpanels.findByIdAndRemove(id, { useFindAndModify: false })
 .then(data => {
 if (!data) {
 res.status(404).send({
...
exports.findAllPublished = (req, res) => {
 Solarpanels.find({ published: true })
.then(data => {
 res.send(data);
 })
.catch(err => {
